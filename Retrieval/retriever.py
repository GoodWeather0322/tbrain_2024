...
class Retriever:
    ...
    該類實現了基於embedding、BM25和LCS的混合檢索

    Attributes:
        ckip_ws (CkipWordSegmenter): 中文ws
        embedding_model (BGEM3FlagModel): 文本embedding模型
    """
    ...
    def _lcs_length(self, query, document):
        """計算兩個序列間的最長公共子序列（LCS）長度。

        Args:
            query (Union[str, list]): 查詢文本或標記列表
            document (Union[str, list]): 文檔文本或標記列表

        Returns:
            float: 標準化後的LCS長度（除以較短序列的長度）
        """
    ...
    def _fusion_retrieve(
        ...
        結合Embedding相似度、BM25分數和LCS分數進行檢索，
        使用shallow fusion策略進行分數融合。

        Args:
            category (str): 類別（finance/insurance/faq）
            qid (str): query ID
            query (str): query
            query_embedding_file (str): 查詢的embedding文件路徑
            source (list): 類別文檔列表
            corpus (dict): 類別文檔
    ...
    def fusion_retrieve(self, questions, dataset):
        ...
        處理多個query，並將結果保存為JSON文件。

        Args:
            questions (list): query列表
            dataset (dict): 類別文檔

        Returns:
            tuple: (答案字典, 答案文件路徑)
                - answer_dict (dict): 包含所有檢索結果的字典
                - answer_dict_path (Path): 結果文件保存路徑
        """
