...
class Retriever:
    ...
    def _fusion_retrieve(
        ...
        lcs_corpus_ids = []
        for file in source:
            lcs_corpus_ids += [file] * len(corpus_bm25[str(file)])
            source_texts = corpus_bm25[str(file)]
        ...
        filtered_corpus = []
        for file in source:
            bm25_corpus_ids += [file] * len(corpus_bm25[str(file)])
            filtered_corpus += corpus_bm25[str(file)]
        tokenized_corpus = filtered_corpus
        bm25 = BM25Okapi(tokenized_corpus)  # 使用BM25演算法建立檢索模型
        tokenized_query = query_bm25[0]  # 將查詢語句進行分詞
        ...
        assert len(bm25_scores) == len(bm25_corpus_ids)
        assert len(corpus_ids) == len(bm25_corpus_ids)
        assert len(lcs_corpus_ids) == len(lcs_scores)
        assert len(lcs_corpus_ids) == len(corpus_ids)
        ...
        # 融合
        fusion_scores = []
        for i, (bm25_score, lcs_score, cosine_score) in enumerate(
            zip(bm25_scores, lcs_scores, cosine_similarity)
        ):
            score = (
