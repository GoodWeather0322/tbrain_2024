...
class DataLoader:
    ...
    def preprocess_dataset(self, questions: list, related_corpus_ids: dict):
        ...
        dataset_json_path = (
            ...
            / f"{corpus_name}_embedding_{settings.reranker}_{settings.max_tokens}_{settings.stride}.json"
        )

        question_json_name = "questions_v9"
        if settings.clean_text:
        ...
        question_json_path = (
            ...
            / f"{question_json_name}_embedding_{settings.reranker}_{settings.max_tokens}_{settings.stride}.json"
        )
        ...
        for category, corpus_ids in related_corpus_ids.items():
            for corpus_id in corpus_ids:
                with open(
                    ...
                    tokens = self.reranker.tokenizer.encode(
                        text, add_special_tokens=False
                    )
                    split_texts = []
                    for i in range(0, len(tokens), settings.stride):
                        split_text = self.reranker.tokenizer.decode(
        ...
        for question in questions:
            ...
            tokens = self.reranker.tokenizer.encode(query, add_special_tokens=False)
            if len(tokens) > 8192:
                print(f"query length: {len(tokens)} exceed 8192, truncate to 8192")
                query = self.reranker.tokenizer.decode(tokens[:8192])
            query = query.replace(" ", "")
