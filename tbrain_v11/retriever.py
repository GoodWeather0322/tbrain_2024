...
from tbrain_v11.settings import settings
...
class Retriever:
    ...
    def _fusion_retrieve(
        ...
        if score_file_path.with_suffix(".npz").exists():
            ...
            # 為每個real corpus取前n名
            unique_corpus_ids = np.unique(corpus_ids)
            top_n_indices = []
            for corpus_id in unique_corpus_ids:
                corpus_mask = np.array(corpus_ids) == corpus_id
                corpus_similarities = cosine_similarity[corpus_mask]
                corpus_indices = np.where(corpus_mask)[0]
                # 取該corpus的前n個最高分數的索引
                top_n_local_indices = np.argsort(corpus_similarities)[-settings.top_n :]
                top_n_indices.extend(corpus_indices[top_n_local_indices])
